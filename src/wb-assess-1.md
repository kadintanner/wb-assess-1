...
```
console.log is for us to check our work, return will end a function and say what the output of the function is.
```

**_What's the difference between defining and calling a function?_**

```
defining means to write out all the code inside a function, calling means to run the code inside the function.
```
...
```
strings let us write plain old english as long as it is surrounded by quotation marks.
```

**_Number_**

```
any number we want, positive or negative.
```

**_Boolean_**

```
it is a true or false datatype.
```

**_Array_**

```
arrays are a datatype thats let us list out multiple items.
```
